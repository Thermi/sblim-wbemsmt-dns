...
public final class Linux_DnsServiceSettingDataHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<Linux_DnsServiceSettingData.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsServiceSettingData.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsServiceSettingData.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsServiceSettingData.Java_Package_List.setElementAt((String)(Linux_DnsServiceSettingData.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsServiceSettingData.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_DnsServiceSettingData getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsServiceSettingDataHelper.getInstance(cimClient, new Linux_DnsServiceSettingData(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsServiceSettingData getInstance(CIMClient cimClient, Linux_DnsServiceSettingData dataInstance) {
		...
		return Linux_DnsServiceSettingDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsServiceSettingData getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_DnsServiceSettingData(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsServiceSettingData createInstance(CIMClient cimClient, Linux_DnsServiceSettingData dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsServiceSettingData newDataInstance = null;
		try {
			newDataInstance = Linux_DnsServiceSettingDataHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsServiceSettingData modifyInstance(CIMClient cimClient, Linux_DnsServiceSettingData dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsServiceSettingDataHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsServiceSettingData dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsServiceSettingData dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsServiceSettingData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsServiceSettingData.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsServiceSettingData.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsServiceSettingData.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsServiceSettingData.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ConfigurationDirectory(String ConfigurationDirectory) {
		return true;
	}
	...
	public static boolean isValid_XferFormat(UnsignedInt8 XferFormat) {
		return true;
	}
