...
public final class Linux_DnsResourceRecordHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<Linux_DnsResourceRecord.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsResourceRecord.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsResourceRecord.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsResourceRecord.Java_Package_List.setElementAt((String)(Linux_DnsResourceRecord.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsResourceRecord.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_DnsResourceRecord getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsResourceRecordHelper.getInstance(cimClient, new Linux_DnsResourceRecord(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsResourceRecord getInstance(CIMClient cimClient, Linux_DnsResourceRecord dataInstance) {
		...
		return Linux_DnsResourceRecordHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsResourceRecord getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_DnsResourceRecord(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsResourceRecord createInstance(CIMClient cimClient, Linux_DnsResourceRecord dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsResourceRecord newDataInstance = null;
		try {
			newDataInstance = Linux_DnsResourceRecordHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsResourceRecord modifyInstance(CIMClient cimClient, Linux_DnsResourceRecord dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsResourceRecordHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsResourceRecord dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsResourceRecord dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsResourceRecord.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsResourceRecord.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsResourceRecord.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsResourceRecord.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsResourceRecord.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Family(UnsignedInt8 Family) {
		return true;
	}
	...
	public static boolean isValid_Type(String Type) {
		return true;
	}
	...
	public static boolean isValid_ZoneName(String ZoneName) {
		return true;
	}
