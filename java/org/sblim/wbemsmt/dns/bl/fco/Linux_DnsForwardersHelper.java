...
public final class Linux_DnsForwardersHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<Linux_DnsForwarders.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsForwarders.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsForwarders.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsForwarders.Java_Package_List.setElementAt((String)(Linux_DnsForwarders.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsForwarders.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_DnsForwarders getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsForwardersHelper.getInstance(cimClient, new Linux_DnsForwarders(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsForwarders getInstance(CIMClient cimClient, Linux_DnsForwarders dataInstance) {
		...
		return Linux_DnsForwardersHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsForwarders getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_DnsForwarders(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsForwarders createInstance(CIMClient cimClient, Linux_DnsForwarders dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsForwarders newDataInstance = null;
		try {
			newDataInstance = Linux_DnsForwardersHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsForwarders modifyInstance(CIMClient cimClient, Linux_DnsForwarders dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsForwardersHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsForwarders dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsForwarders dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsForwarders.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsForwarders.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsForwarders.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsForwarders.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsForwarders.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ForwardersElementType(UnsignedInt8[] ForwardersElementType) {
		return true;
	}

	public static boolean isValid_ForwardersElement(String[] ForwardersElement) {
		return true;
	}
