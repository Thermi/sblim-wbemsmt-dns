...
public final class Linux_DnsAllowUpdateForZoneHelper {

	public final static String CIM_CLASS_NAME = "Linux_DnsAllowUpdateForZone";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DnsAllowUpdateForZone(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_DnsAllowUpdateForZone.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsAllowUpdateForZone.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsAllowUpdateForZone.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsAllowUpdateForZone.Java_Package_List.setElementAt((String)(Linux_DnsAllowUpdateForZone.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsAllowUpdateForZone.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_DnsAllowUpdateForZone getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsAllowUpdateForZoneHelper.getInstance(cimClient, new Linux_DnsAllowUpdateForZone(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsAllowUpdateForZone getInstance(CIMClient cimClient, Linux_DnsAllowUpdateForZone dataInstance) {
		...
		return Linux_DnsAllowUpdateForZoneHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsAllowUpdateForZone getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_DnsAllowUpdateForZone(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsAllowUpdateForZone createInstance(CIMClient cimClient, Linux_DnsAllowUpdateForZone dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsAllowUpdateForZone newDataInstance = null;
		try {
			newDataInstance = Linux_DnsAllowUpdateForZoneHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsAllowUpdateForZone modifyInstance(CIMClient cimClient, Linux_DnsAllowUpdateForZone dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsAllowUpdateForZoneHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsAllowUpdateForZone dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsAllowUpdateForZone dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsAllowUpdateForZone.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsAllowUpdateForZone.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsAllowUpdateForZone.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsAllowUpdateForZone.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsAllowUpdateForZone.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
