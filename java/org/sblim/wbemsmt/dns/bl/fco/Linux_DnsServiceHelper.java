...
public final class Linux_DnsServiceHelper {
	...
	*/	
	public final static Linux_DnsService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsServiceHelper.getInstance(cimClient, new Linux_DnsService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsService getInstance(CIMClient cimClient, Linux_DnsService dataInstance) {
		...
		return Linux_DnsServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DnsService(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DnsService.getPackages();
	...
	*/	
	public final static Linux_DnsService createInstance(CIMClient cimClient, Linux_DnsService dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsService newDataInstance = null;
		try {
			newDataInstance = Linux_DnsServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsService modifyInstance(CIMClient cimClient, Linux_DnsService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_RunAsRoot(Boolean RunAsRoot) {
		return true;
	}
