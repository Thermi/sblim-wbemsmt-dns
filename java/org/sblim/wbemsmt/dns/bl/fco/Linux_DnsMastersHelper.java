...
public final class Linux_DnsMastersHelper {
	...
	*/	
	public final static Linux_DnsMasters getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsMastersHelper.getInstance(cimClient, new Linux_DnsMasters(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsMasters getInstance(CIMClient cimClient, Linux_DnsMasters dataInstance) {
		...
		return Linux_DnsMastersHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsMasters getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_DnsMasters(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsMasters createInstance(CIMClient cimClient, Linux_DnsMasters dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsMasters newDataInstance = null;
		try {
			newDataInstance = Linux_DnsMastersHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsMasters modifyInstance(CIMClient cimClient, Linux_DnsMasters dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsMastersHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsMasters dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsMasters dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsMasters.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsMasters.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsMasters.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsMasters.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsMasters.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_MasterElementType(UnsignedInt8[] MasterElementType) {
		return true;
	}

	public static boolean isValid_MasterElement(String[] MasterElement) {
		return true;
	}
