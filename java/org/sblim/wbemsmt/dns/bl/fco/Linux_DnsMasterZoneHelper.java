/** 
 * Linux_DnsMasterZoneHelper.java
 *
...
public final class Linux_DnsMasterZoneHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DnsMasterZone(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_DnsMasterZone.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsMasterZone.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsMasterZone.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsMasterZone.Java_Package_List.setElementAt((String)(Linux_DnsMasterZone.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsMasterZone.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class Linux_DnsMasterZone.");
						resultArrayList.add(new Linux_DnsMasterZone(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class Linux_DnsMasterZone.");
						resultArrayList.add(new Linux_DnsMasterZone(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class Linux_DnsMasterZone.");
						resultArrayList.add(new Linux_DnsMasterZone(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DnsMasterZone getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsMasterZoneHelper.getInstance(cimClient, new Linux_DnsMasterZone(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsMasterZone getInstance(CIMClient cimClient, Linux_DnsMasterZone dataInstance) {
		...
		return Linux_DnsMasterZoneHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsMasterZone getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		Linux_DnsMasterZone dataInstance = null;
		try {
			dataInstance = new Linux_DnsMasterZone(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsMasterZone createInstance(CIMClient cimClient, Linux_DnsMasterZone dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsMasterZone newDataInstance = null;
		try {
			newDataInstance = Linux_DnsMasterZoneHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsMasterZone modifyInstance(CIMClient cimClient, Linux_DnsMasterZone dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsMasterZoneHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsMasterZone dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsMasterZone dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsMasterZone.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsMasterZone.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsMasterZone.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsMasterZone.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsMasterZone.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Contact(String Contact) {
		return true;
	}

	public static boolean isValid_Expire(UnsignedInt32 Expire) {
		return true;
	}

	public static boolean isValid_Forward(UnsignedInt8 Forward) {
		return true;
	}
	...
	public static boolean isValid_NegativeCachingTTL(UnsignedInt32 NegativeCachingTTL) {
		return true;
	}

	public static boolean isValid_Refresh(UnsignedInt32 Refresh) {
		return true;
	}

	public static boolean isValid_Retry(UnsignedInt32 Retry) {
		return true;
	}
	...
	public static boolean isValid_Server(String Server) {
		return true;
	}

	public static boolean isValid_TTL(Integer TTL) {
		return true;
	}

	public static boolean isValid_Type(UnsignedInt8 Type) {
		return true;
	}

	public static boolean isValid_ZoneFile(String ZoneFile) {
		return true;
	}
