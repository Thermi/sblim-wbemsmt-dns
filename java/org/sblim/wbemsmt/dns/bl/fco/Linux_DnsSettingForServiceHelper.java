/** 
 * Linux_DnsSettingForServiceHelper.java
 *
...
public final class Linux_DnsSettingForServiceHelper {

	public final static String CIM_CLASS_NAME = "Linux_DnsSettingForService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DnsSettingForService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_DnsSettingForService.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsSettingForService.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsSettingForService.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsSettingForService.Java_Package_List.setElementAt((String)(Linux_DnsSettingForService.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsSettingForService.Java_Package_List.get(i)) + cimClassName;
					...
					if (clazz == null) {
						System.err.println("The class " + cimInstance.getClassName() +" was not found. Constructing instance of class Linux_DnsSettingForService.");
						resultArrayList.add(new Linux_DnsSettingForService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The required constructor of class " + cimInstance.getClassName() + " could not be found. Constructing instance of class Linux_DnsSettingForService.");
						resultArrayList.add(new Linux_DnsSettingForService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					...
					try {
						...
						System.err.println("The instance of class " + cimInstance.getClassName() + " could not be created successful. Constructing instance of class Linux_DnsSettingForService.");
						resultArrayList.add(new Linux_DnsSettingForService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DnsSettingForService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsSettingForServiceHelper.getInstance(cimClient, new Linux_DnsSettingForService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsSettingForService getInstance(CIMClient cimClient, Linux_DnsSettingForService dataInstance) {
		...
		return Linux_DnsSettingForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsSettingForService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		Linux_DnsSettingForService dataInstance = null;
		try {
			dataInstance = new Linux_DnsSettingForService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsSettingForService createInstance(CIMClient cimClient, Linux_DnsSettingForService dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsSettingForService newDataInstance = null;
		try {
			newDataInstance = Linux_DnsSettingForServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsSettingForService modifyInstance(CIMClient cimClient, Linux_DnsSettingForService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsSettingForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsSettingForService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsSettingForService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsSettingForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsSettingForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsSettingForService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsSettingForService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsSettingForService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
