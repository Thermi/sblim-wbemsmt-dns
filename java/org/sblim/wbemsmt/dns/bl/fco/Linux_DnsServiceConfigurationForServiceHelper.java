...
public final class Linux_DnsServiceConfigurationForServiceHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DnsServiceConfigurationForService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_DnsServiceConfigurationForService.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsServiceConfigurationForService.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsServiceConfigurationForService.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsServiceConfigurationForService.Java_Package_List.setElementAt((String)(Linux_DnsServiceConfigurationForService.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsServiceConfigurationForService.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_DnsServiceConfigurationForService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsServiceConfigurationForServiceHelper.getInstance(cimClient, new Linux_DnsServiceConfigurationForService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsServiceConfigurationForService getInstance(CIMClient cimClient, Linux_DnsServiceConfigurationForService dataInstance) {
		...
		return Linux_DnsServiceConfigurationForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsServiceConfigurationForService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_DnsServiceConfigurationForService(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsServiceConfigurationForService createInstance(CIMClient cimClient, Linux_DnsServiceConfigurationForService dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsServiceConfigurationForService newDataInstance = null;
		try {
			newDataInstance = Linux_DnsServiceConfigurationForServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsServiceConfigurationForService modifyInstance(CIMClient cimClient, Linux_DnsServiceConfigurationForService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsServiceConfigurationForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsServiceConfigurationForService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsServiceConfigurationForService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsServiceConfigurationForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsServiceConfigurationForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsServiceConfigurationForService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsServiceConfigurationForService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsServiceConfigurationForService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DnsService(Linux_DnsService Linux_DnsService) {
		return true;
	}

	public static boolean isValid_Linux_DnsServiceConfiguration(Linux_DnsServiceConfiguration Linux_DnsServiceConfiguration) {
		return true;
	}
