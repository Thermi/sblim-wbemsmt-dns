...
public final class Linux_DnsMastersForZoneHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DnsMastersForZone(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_DnsMastersForZone.Java_Package_List.size(); i++) {
						if (!((String)(Linux_DnsMastersForZone.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_DnsMastersForZone.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_DnsMastersForZone.Java_Package_List.setElementAt((String)(Linux_DnsMastersForZone.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_DnsMastersForZone.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_DnsMastersForZone getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DnsMastersForZoneHelper.getInstance(cimClient, new Linux_DnsMastersForZone(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsMastersForZone getInstance(CIMClient cimClient, Linux_DnsMastersForZone dataInstance) {
		...
		return Linux_DnsMastersForZoneHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DnsMastersForZone getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_DnsMastersForZone(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsMastersForZone createInstance(CIMClient cimClient, Linux_DnsMastersForZone dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DnsMastersForZone newDataInstance = null;
		try {
			newDataInstance = Linux_DnsMastersForZoneHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DnsMastersForZone modifyInstance(CIMClient cimClient, Linux_DnsMastersForZone dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DnsMastersForZoneHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DnsMastersForZone dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DnsMastersForZone dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DnsMastersForZone.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DnsMastersForZone.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DnsMastersForZone.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DnsMastersForZone.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DnsMastersForZone.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
