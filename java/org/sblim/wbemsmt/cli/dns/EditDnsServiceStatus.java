/**
 * EditDnsServiceStatus.java Â© Copyright IBM Corp.  2009,2006,2007 THIS FILE IS PROVIDED UNDER THE TER MS
...
public class EditDnsServiceStatus extends CimCommand {
    ...
    public static final OptionDefinition KEY_restart = new OptionDefinition(null, "restart", null,
            null, false, false, "DnsServiceOperationsDataContainer.restart.argDescription");
    /**
     *  --start
     */
    public static final OptionDefinition KEY_start = new OptionDefinition(null, "start", null,
            null, false, false, "DnsServiceOperationsDataContainer.start.argDescription");
    /**
     *  --stop
     */
    public static final OptionDefinition KEY_stop = new OptionDefinition(null, "stop", null, null,
            false, false, "DnsServiceOperationsDataContainer.stop.argDescription");
    ...
    private static final OptionDefinition[] LOCAL_OPTIONS = new OptionDefinition[] { KEY_restart,
            KEY_start, KEY_stop, };
            ...
            values.getOut().println(
                    "\n"
                            + bundle.getString("editing", new Object[] { bundle
                                    .getString("DnsServiceOperationsDataContainer.caption") }));

            CliDataLoader loader = new EditDnsServiceStatusLoader();
            loader.load(bundle, adapter, commandValues);

            org.sblim.wbemsmt.cli.dns.container.edit.DnsServiceOperationsDataContainerImpl dc = new org.sblim.wbemsmt.cli.dns.container.edit.DnsServiceOperationsDataContainerImpl(
                    adapter);
    ...
    private void setKeyValues(CommandLine cmd, AbstractBaseCimAdapter adapter,
            org.sblim.wbemsmt.dns.bl.container.edit.DnsServiceOperationsDataContainer dc)
    ...
    private void setValues(CommandLine cmd, AbstractBaseCimAdapter adapter,
        ...
        //The Buttons
        if (!pressButton(cmd, adapter, dc, dc.get_usr_Restart(), KEY_restart))
            return;
        if (!pressButton(cmd, adapter, dc, dc.get_invoke_Start(), KEY_start))
            return;
        if (!pressButton(cmd, adapter, dc, dc.get_invoke_Stop(), KEY_stop))
            return;
    }
